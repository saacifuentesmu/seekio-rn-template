...
export const HomeScreen: React.FC = () => {
  ...
  return (
    <View style={[styles.container, {backgroundColor: palette.background, padding: spacing.lg}]}>
      ...
      {me.isPending ? (
        <Text style={[typography.body, {color: palette.textMuted, marginTop: spacing.sm}]}>{t('common.loading')}</Text>
      ) : me.isError ? (
        <View style={{marginTop: spacing.sm}}>
          <Text style={[typography.body, {color: palette.error, marginBottom: spacing.sm}]}>
            {t('home.profileLoadFailed')}
          </Text>
          <Button title={t('common.retry')} onPress={() => me.refetch()} variant="secondary" />
        </View>
      ) : (
